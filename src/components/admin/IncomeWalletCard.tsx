...
export default function IncomeWalletCard({ memberName, memberGroup, totalEarnings, avatar }: Props) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[20px] bg-slate-100 shadow-sm">
      <div className="absolute inset-0"><Image src={imageSource} alt="Member profile" fill className="object-cover" sizes="(max-width: 1120px) 100vw, 1120px" unoptimized /><div className="absolute inset-x-0 bottom-0 h-36 bg-gradient-to-t from-white via-white/90 to-transparent" /></div>
      <div className="relative flex h-full min-h-[500px] flex-col justify-between p-6 pt-5 text-white">
        ...
        <div className="mt-auto flex items-end justify-between gap-4 text-left text-black">
          ...
          <div className="text-right">
            ...
            <p className="mt-2 text-3xl font-semibold">{totalEarnings == null ? '' : `₹${totalEarnings.toLocaleString('en-IN')}`}</p>
          </div>
        </div>
      </div>
    </div>
