...
CREATE TABLE IF NOT EXISTS members (
  ...
  level_income_wallet NUMERIC(14,2) NOT NULL DEFAULT 0,
  mbd_wallet      NUMERIC(14,2)   NOT NULL DEFAULT 0,
  booster_topup   NUMERIC(14,2)   NOT NULL DEFAULT 0,
...
BEGIN
  ...
  INSERT INTO referrals (
    id, sponsor_id, sponsor_name, member_id, member_name,
    level_name, join_date, status, reward_amount
  )
  VALUES (
    'REF-' || NEW.id,
    NEW.sponsor_id,
    ...
    NEW.level_name,
    NEW.joining_date,
    CASE WHEN NEW.status IN ('Active', 'Approved') THEN NEW.status ELSE 'Pending' END,
    0
  )
